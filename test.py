...
def evaluate_batch(loader, encoder, decoder, tgt_max_length, tgt_idx2words, src_idx2words):
    ...
    for input_tensor, input_lengths, target_tensor, target_lengths in val_loader:
        ...
        target_input = (torch.ones([batch_size, tgt_max_length],dtype = torch.long) * SOS_token).to(device)
        decoder_outputs, _ = decoder(target_input,src_lengths,encoder_outputs)
        
        topv, topi = decoder_outputs.topk(1)

        target_tensor_numpy = target_tensor.cpu().numpy()
        input_tensor_numpy = input_tensor.cpu().numpy()
        idx_token_pred = topi.squeeze(2).cpu().detach().numpy()
...
def train(input_tensor, input_lengths, target_tensor, target_lengths,
    ...
    loss = criterion(decoder_outputs.transpose(1,2), target_tensor)
...
def trainIters(train_loader, val_loader, encoder, decoder, num_epochs, learning_rate, srcLang, tgtLang):
...
num_epochs, learning_rate = 25, 1e-4
trainIters(train_loader, val_loader, encoder, decoder, num_epochs, learning_rate, srcLang, tgtLang)
